...
```{r}
dir.create("tracks")
epiwraps::bam2bw("aligned/ctcf.bam", "tracks/ctcf_full.bw", binWidth=1, extend=100, scaling=TRUE) # set "extend" because they are not pair-end
...
```{r}
peaks$overlapsPromoter <- overlapsAny(peaks, proms) # what peaks overlaps with the promoter
peaksInPromoters <- peaks[peaks$overlapsPromoter]
head(peaksInPromoters)
```
